/* ============================================================
   ...
   Setup:
     ...
     npm start
   Then either:
     a) open http://localhost:3000 directly (this server also serves
        the game's static files), or
     b) deploy this server somewhere public (Render/Railway/Fly.io/a
        VPS) and, on the standalone single-file game, enter that
        server's URL on the "Play Online Multiplayer" screen and
        click Connect before creating/joining a room.

   Responsibilities:
     - Serve the game's static files (index.html, css, js) — optional,
       only used if you open this server's own URL directly.
...
// from the static game file (e.g. game on a static host, server on
// Render/Railway). Tighten `origin` to your real domain in production.
const io = new Server(server, { cors: { origin: '*' } });

const PORT = process.env.PORT || 3000;

// Serve the game files from this same directory (optional convenience).

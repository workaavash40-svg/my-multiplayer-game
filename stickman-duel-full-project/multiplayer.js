/* ============================================================
   multiplayer.js  (CLIENT SIDE)
   Online multiplayer using Socket.IO. Requires server.js running
   somewhere reachable (see server.js) and the Socket.IO client
   ...
   - The player who creates the room is the HOST and runs the real
     physics simulation locally (same Game.update() used offline).
   ...
   - Character color (Blue/Red, i.e. which of Game.p1/p2 you are) is
     ...
     always plays with the same Blue/WASD key layout locally — see
     Game.update()'s `mine`/`theirs` routing.
   ============================================================ */
...
const Multiplayer = {
  ...
  connect() {
    ...
    // Empty string -> same-origin (works when this page is served BY
    // server.js). Otherwise connect to whatever URL the player entered
    // (e.g. a server they deployed to Render/Railway/a VPS).
    ...
    this.socket.on('connect_error', (err) => {
      ...
      this.setStatus(`Could not reach server (${err.message || 'connection error'}). Check the server URL and that server.js is running.`);
    });
    ...
    this.socket.on('chat-message', ({ text, mine }) => {
      UI.addChatMessage(text, mine ? 'me' : 'them');
    });
  },
  ...
  // Called once per frame from game.js when Game.mode === 'online'.
  ...
  applyState(state) {
    ...
    if (state.matchOver && this.game.state !== 'matchover') {
      ...
      SFX.play('victory');
      UI.setVictoryOnline(amIWinner, winner.color, this.game.p1.score, this.game.p2.score);
